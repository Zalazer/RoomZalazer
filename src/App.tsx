...
const r=await api("/auth/register",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name,email,password})})
if(!r.ok)return alert(r.error)
alert("Registered.")
...
title,
description:"",
start_at:`${selectedDate}T${start}:00Z`,
end_at:`${selectedDate}T${end}:00Z`
}

const r=await api("/reservations",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)})
if(!r.ok)return alert(r.error)
...
<button className="primary" onClick={registerMode?register:login}>{registerMode?"Register":"Login"}</button>
<button className="secondary" onClick={()=>setRegisterMode(!registerMode)}>{registerMode?"Back to Login":"Create Account"}</button>
</div>
</div>
)
...
<HeaderCard time={time} rooms={rooms.length} reservations={myReservations.length} user={user?.name} onLogout={logout}/>
...
{rooms.map(room=>(
<RoomCard key={room.id} room={room} reservations={reservations} selectedDate={selectedDate} onClick={()=>setSelectedRoom(room)}/>
))}
...
<button className="float" onClick={()=>setShowModal(true)}>+</button>
